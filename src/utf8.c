...
from_utf8_to_utf16le(int infile, int outfile)
{
  int ret = 0;
  int bom;
  utf8_glyph_t utf8_buf; //4 bytes
  ssize_t bytes_read; //8 bytes
  size_t remaining_bytes; //8 bytes
  size_t size_of_glyph;
  code_point_t code_point; // 4 bytes
  utf16_glyph_t utf16_buf; //4 bytes
  ...
  while((bytes_read = read_to_bigendian(infile, &utf8_buf.bytes[0], 1)) > 0) {
    ...
    utf8_buf.bytes[0].byte = 0;
    utf8_buf.bytes[1].byte = 0;
    utf8_buf.bytes[2].byte = 0;
    utf8_buf.bytes[3].byte = 0;
  }
  ret = bytes_read;
  return ret;
}
...
get_utf8_decoding_function(size_t size)
{
  switch(size) {
  case 1:
    return (utf8_decoding_func_t)utf8_one_byte_decode;
  case 2:
    return (utf8_decoding_func_t)utf8_two_byte_decode;
  case 3:
    return (utf8_decoding_func_t)utf8_three_byte_decode;
  case 4:
    return (utf8_decoding_func_t)utf8_four_byte_decode;
  }
  return NULL;
}
